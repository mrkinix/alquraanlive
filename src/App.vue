...
export default {
  data() {
    return {
      ...
      currentTheme: 'dark',
      // Constants
      QURAN_AR_FILE: 'quran.json',
      QURAN_EN_FILE: 'en.json',
      WORDS_PER_PART: 25,
  ...
  computed: {
    ...
    isWordCorrect() {
      return (index) => {
        ...
        return this.correctWords.includes(index);
      };
    },
    isWordMistake() {
      return (index) => {
        if (!this.isRevisionMode) return false;
        return this.mistakeWords.includes(index);
      };
    },
    isWordUnread() {
      return (index) => {
        if (!this.isRevisionMode) return false;
        return this.unreadVerses.includes(index);
      };
    }
  },
    ...
    async loadSurahVerses(surahId) {
      try {
        ...
        if (data.code === 200) {
          ...
          this.errorMessage = this.currentLanguage === 'ar' ? 'فشل تحميل السورة.' : 'Failed to load Surah.';
        }
      } catch (error) {
    ...
    async initAudioPlayback() {
      ...
      try {
        ...
        if (this.mushafType === 'hafs') {
          ...
          // Fallback to Hafs audio for Warsh/Qaloon
